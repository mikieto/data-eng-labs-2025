...
In later chapters, we let AI propose changes as
`ai_generated_change_pack` JSON objects, instead of letting AI edit
files directly. Those change packs are then evaluated by guards before
any real system is updated.
...
- **Inputs**
  ...
  - `labs/ch07/inputs/ai_generated_change_pack_example.json`  
    ...
    would propose, based on the snapshot and STR cards).
...
- **Design note (optional reading)**
  ...
  - `labs/ch07/CH07_lab_min_spec.md`  
    ...
    how CH07 fits into the broader STR → snapshot → change pack
    pattern.
...
## Step 4 — From this lab to real systems
...
* The **AI-generated change pack** is produced by an external LLM
  (plus human review), based on that snapshot and a STR card.
